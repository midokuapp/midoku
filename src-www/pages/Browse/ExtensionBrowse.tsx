...
export default function ExtensionBrowse() {
    ...
    useEffect(() => {
        const handleScroll = () => {
            if (document.body.scrollHeight - 300 < globalThis.scrollY + globalThis.innerHeight) {
                if (!loading) {
...
const ErrorMessage = ({error}: { error: string }) => (<div style={{padding: "0 2rem"}}>
...
const ExtensionHeader = ({extension}: { extension: Extension }) => (<div
...
// Grille de mangas
const MangaGrid = ({mangas, extensionId}: { mangas: Array<Manga>; extensionId: string },) => (<ul
        style={{
            display: "grid",
    ...
    >
        {mangas.map((manga) => (<li key={manga.id} style={{textAlign: "center"}}>
                <Link
